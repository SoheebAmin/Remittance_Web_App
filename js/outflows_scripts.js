...
var arr = []
var full_arr = [];
for (let i = 0; i != outflows_data.length; i++) {
    current_country = Object.values(outflows_data[i])
...
for (let i = 0; i != top_5_remittance.length; i++) {
    ...
    for (let i = 0; i != outflows_data.length; i++) {
        current_country_data = Object.values(outflows_data[i]);
        if(country_to_get == current_country_data[47]) {
        ...
        for (let j = 0; j != current_country.length; j++) {
            if(typeof current_country[j] === 'string') {
                if(current_country[j].toLowerCase() == input.toLowerCase()) {
                    country_with_data = outflows_data[i]
